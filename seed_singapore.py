...
def build_and_push_agent(
    ...
    user_prompt = (
        ...
        f"Ground this agent in authentic Singaporean upper middle class experience -- "
        f"elite school streaming and pressure (Primary School Leaving Exam, O/A-levels, "
        f"polytechnic vs JC vs university debates), National Service for males, "
        f"the 5Cs aspiration (cash, car, credit card, condo, country club), "
        f"kiasu culture and the fear of falling behind, "
        f"HDB vs private property identity, the negotiation between "
        f"Singlish and formal English registers, "
        f"racial harmony rhetoric vs everyday ethnic friction, "
        f"and the tension between filial piety and personal ambition in a hyper-meritocratic city-state.\n"
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"  [Agent {agent_index}] JSON parse error: {e}. Retrying...")
            time.sleep(2)
        except anthropic.RateLimitError:
            wait = 30
            print(f"  [Agent {agent_index}] Rate limited. Waiting {wait}s...")
            time.sleep(wait)
        except anthropic.APIStatusError as e:
            if e.status_code == 529:
                wait = 15 * attempt
                print(f"  [Agent {agent_index}] Anthropic overloaded (529). Waiting {wait}s...")
                time.sleep(wait)
            else:
...
def main():
    ...
    target_demographic = "Singaporean Upper Middle Class"
    region = "Singapore"
    total_agents = 5
    delay_between = 1
